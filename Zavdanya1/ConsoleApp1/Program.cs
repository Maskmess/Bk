...
class Bank
{
    ...
    public BankAccount GetAccount(string accountNumber, int pin)
    {
        if (accounts.ContainsKey(accountNumber))
        {
            ...
            // Якщо pin дорівнює 0, не перевіряємо PIN (використовується для перевірки отримувача)
            if (pin == 0 || account.Pin == pin)
            {
                return account;
            }
            else
...
class Program
{
    static void Main()
    {
        // Встановлюємо кодування на UTF-8 для коректного відображення українських символів
            ...
            switch (option)
            {
                ...
                case "4":
                    ...
                    if (senderAccount != null)
                    {
                        ...
                        // Перевірка існування рахунку отримувача БЕЗ перевірки PIN
                        var receiverAccount = bank.GetAccount(receiverAccountNumber, 0); // PIN не перевіряється для отримувача
